...
#% multiple: no
#% key_desc: order
#% description: Order of values in .sub file (; for linebreak, variables as given as arguments, flu=subbasin area fraction)
#% answer: flu,dummy,salb,sno,chl,chs,chw,chk,chn,ovn;rt,css,ecp,sl,stp
#%end
...
#% multiple: no
#% key_desc: order
#% description: Order of values in .rte file (; for linebreak, see help for values)
...
#% multiple: no
#% key_desc: order
#% description: Order of values in .gw file (; for linebreak, see help for values)
...
class main:
    def __init__(self,**optionsandflags):
        ...
        for f in self.orders:
            ...
            # make lists
            self.orders[f] = re.split(',|;',self.orders[f])
...
Can only find/calculate %s values for %s, but there are %s subbasins.""" %(len(par),p,self.nsubbasins))
    ...
    def correctChannelLength(self):
        ...
        grun('v.db.addcolumn',map=self.subbasins,columns='perim__',quiet=True)
    ...
    def writeSubFiles(self,data,linebreak=';'):
        ...
        # read orders with line breaks
        orders = {'sub':self.suborder.split(linebreak),
                  'gw' :self.gworder.split(linebreak),
                  'rte':self.rteorder.split(linebreak)}
        # build up formats
        fmts = {}
        for p in orders:
            fmts[p] = 'SUBBasin %s DATA {id} {nextid}\n' %p
            for l in orders[p]:
                fmts[p] += ' '.join(['{%s:14.5f}' %s for s in l.split(',')])+'\n'
            gm('Format for %s: \n%s' %(p,fmts[p]))
        # make name
        ilen = len(str(self.nsubbasins)) # integer length
        fname = os.path.join(self.subpath,self.projectname+'%0'+str(ilen)+'i.%s') # file numbering
        grass.message('Writing .sub, .gw and .rte files to %s' %self.subpath)
        # loop over number of subbasins and write out
        for i in range(self.nsubbasins):
            # make dict of values
            inserter = {}
            for e in data:
                inserter[e]=data[e][i]
            inserter.update({'id':i+1,'nextid':i+2})
            for p in fmts:
                fi = file(fname %(i+1,p),'w')
                fi.write(fmts[p].format(**inserter))
                fi.close()
        return
    ...
    def writeFileCio(self):
        ...
        headerlines = [[]]*3 + [  # number of empty lines at start
                       [self.projectname+ex for ex in ['.cod','.fig','.str','.bsn']]+['CNtab.dat'],
                       ('crop.dat','agman.dat','wgen.dat'),
                       ('soil.cio','str.cio'),
                       ('prec.dat',),
                       ('temp.dat',),
                       ('radi.dat',),
                       ('runoff.dat',) ] + [[]]*4 # number of empty lines at end
        # write header
